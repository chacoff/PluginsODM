...
export default class App{
    constructor(map){
          ...
          mapTempLayerDrop.on("addedfile", (file) => {
            ...
            reader.onload = (event) => {
              try {
                ...
                } else { myToast.showToast('Fichier importé avec succès', 'success'); }
              } catch (error) {
        ...
        function clearAllLayer() {
          ...
          map.eachLayer(layer => {
            if (!layer._url && typeof layer.remove === 'function') {
        ...
        function saveFile(id) {
          ...
          map.eachLayer(layer => {
              ...
              console.log(mp)
              if (mp) {
                  const geoJSONFeature = mp.getGeoJSON(); // Récupère les données GeoJSON de la mesure
                  // Vérifiez si le modèle existe et ajoutez le titre si disponible
                  console.log("Model geojson", mp, geoJSONFeature, mp.props.model)
                  if (mp.props.model) {
                  ...
                  // Ajoutez la caractéristique GeoJSON à la collection
                  features.push(geoJSONFeature);
              }
          });

          // Construisez l'objet GeoJSON complet
          const geoJSON = {
              type: "FeatureCollection",
              features: features
          };

          // Sauvegardez le fichier GeoJSON
          if (id != null) {
            $.ajax({
              type: 'POST',
              url: `/api/plugins/VolumePlugingODM/task/file/save/${id}`,
              data: JSON.stringify({
                geoJSON: geoJSON,
                name: id,
              }),
              ...
              if (!result['error']) {
                myToast.showToast('Les infos ont été sauvgardé avec succès', 'success');
                return
              } else {
                myToast.showToast("Un problème est survenue pendant la sauvegarde", 'error');
              }
            })
          } else {
          ...
          for (let l in map._layers){
            ...
            if (layer.options && layer.options.bounds){
              ...
              if (metaSymbol) {
                  ...
                  console.log("Meta name :", meta.task.name); 
              } else {
